namespace HyperGames.UnityTemplate.UnityTemplate.Services
{
    ...
    public abstract class BaseUnityNotificationService : INotificationService, IDisposable
    {
        ...
        protected readonly UnityTemplateNotificationBlueprint     UnityTemplateNotificationBlueprint;
        protected readonly UnityTemplateNotificationDataBlueprint UnityTemplateNotificationDataBlueprint;
        ...
        )
        {
            ...
            this.UnityTemplateNotificationBlueprint     = unityTemplateNotificationBlueprint;
            this.UnityTemplateNotificationDataBlueprint = unityTemplateNotificationDataBlueprint;
        ...
        protected void TrackEventClick(NotificationContent content)
        {
            ...
            this.Logger.Log($"mirailog: Notification Opened: {content.Title} - {content.Body}");
        }
        ...
        public async void SetUpNotification()
        {
            ...
            try
            {
                ...
                var UnityTemplateNotificationRecords = this.UnityTemplateNotificationBlueprint.Values.Where(x => x.RandomAble).ToList();

                foreach (var notificationData in UnityTemplateNotificationRecords) tasks.Add(this.PrepareRemind(notificationData));

                var notificationContents = await UniTask.WhenAll(tasks).AttachExternalCancellation(this.ctsSetupNotification.Token);
                for (var i = 0; i < UnityTemplateNotificationRecords.Count; i++)
                {
                    var notificationData = UnityTemplateNotificationRecords[i];
        ...
        private void ScheduleNotification(UnityTemplateNotificationRecord notificationData, TimeSpan delayTime, NotificationContent notificationContent)
        {
            ...
            this.Logger.Log($"mirailog: Notification Schedule: {notificationContent.Title} - {notificationContent.Body} - {fireTime}");
        ...
        private async UniTask<NotificationContent> PrepareRemind(UnityTemplateNotificationRecord record)
        {
            var title = "";
            var body  = "";

            var listCanRandom = this.UnityTemplateNotificationDataBlueprint.Values.Where(x => x.RandomAble).ToList();
            var itemRandom    = listCanRandom.Count > 0 ? listCanRandom.PickRandom() : null;

            if (itemRandom == null) this.Logger.Warning($"There is no item can random in {this.UnityTemplateNotificationDataBlueprint.GetType().Name}, ignore random");
            ...
            {
                title = await this.NotificationMappingHelper.GetFormatString(this.UnityTemplateNotificationDataBlueprint[record.Title].Title);
            }
            ...
            {
                body = await this.NotificationMappingHelper.GetFormatString(this.UnityTemplateNotificationDataBlueprint[record.Body].Body);
            }
        ...
        {
            if (!this.UnityTemplateNotificationBlueprint.TryGetValue(notificationId, out var notification)) return;
            if (!this.UnityTemplateNotificationDataBlueprint.TryGetValue(notificationId, out var notificationData)) return;
